...
public class CollegeOrSchoolController : APIBaseController
{
    private readonly IDapper dapper;
    private readonly IErrorLogger errorLogger;

    public CollegeOrSchoolController(IDapper dapper, IErrorLogger errorLogger)
    {
        this.dapper = dapper;
    ...
    public async Task<IActionResult> Get(int stream,int qualificationType, int districtid ,int? LangId)
    {
        try
        {
            ...
            else
            {
                ...
                result = await Task.FromResult(dapper.GetAll<SelectListModel>("USP_GetCollegeOrSchoolList", myparam, commandType: System.Data.CommandType.StoredProcedure));
            }
            return Ok(result);
        }
        catch (Exception ex)
        {
            ...
            await errorLogger.Log("usp_FilterSchoolsForPostPast/ USP_GetCollegeOrSchoolList", ex);
