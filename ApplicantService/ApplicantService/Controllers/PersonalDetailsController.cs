...
public class PersonalDetailsController : APIBaseController
{
    ...
    public async Task<IActionResult> Post([FromBody] PersonalDetailsModel model)
    {
        ...
        try
        {
            if (!string.IsNullOrEmpty(model.DOB) && model.IsNewApplicant)
            {
                DateTime currentDate = DateTime.Now;
                if (DateTime.TryParse(model.DOB, out DateTime date))
                {
                    ...
                    //currentDate > date.AddYears(30))
                    if (age == 30 && (currentDate.Month > date.Month || (currentDate.Month == date.Month && currentDate.Day > date.Day)))
                    {
                        age++;
                    }
                    if (age > 30)
                    {
                        return Ok(new { Status = false, Message = "Personal details cannot be saved due to their age exceeding 30 years." });
                    }
                }
            }

            bool checkCastewiseElgIbility = await profileService.getCastewiseElgIbility(model.CasteCategory);
            ...
            if (!string.IsNullOrWhiteSpace(result.err_message))
                return Ok(new { Status = false, Message = result.err_message });
            else if (!string.IsNullOrWhiteSpace(result.ApplicantName))
