...
public interface ICurrentCourseService
{
    ...
    Task<string> CalculateYearGap(int courseYrId,string selectedYear);
    Task<VerifyYearsModel> CheckIsDatainYearOrder(int CourseId, int YearStudy);
   ...
   Task<int> SaveApplicantPreSchoolRecord(ApplicantPreSchoolRecord model);
    ...
    Task<int> CheckIsCurrentAvailable();
    ...
    Task<string> GetVerifyHostel(int district);
    Task<string> UpdateServiceType(int district);
}
