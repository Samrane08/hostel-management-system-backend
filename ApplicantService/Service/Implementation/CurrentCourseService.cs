...
public class CurrentCourseService : ICurrentCourseService
{
    ...
    public async Task<string> SaveCurrentCourseDetailsAsync(CurrentCourseDetails model)
    {
        try
        {
            ...
            myparam.Add("_VillageID", model.VillageID, DbType.Int32);
    ...
    public async Task<string> CalculateYearGap(int courseYrId,string this_year)
    {
        try
        {
            var myparam = new DynamicParameters();
            myparam.Add("_UserId", currentUserService.UserNumericId, DbType.Int64);
            myparam.Add("This_Year", this_year, DbType.Int64);
            myparam.Add("CurrentAcademicYear", courseYrId, DbType.Int64);
           // myparam.Add("AdmissionDate", admissionDate, DbType.Date);
    ...
    public async Task<VerifyYearsModel> CheckIsDatainYearOrder(int CourseId, int YearStudy)
    {
        try
        {

            VerifyYearsModel obj=new VerifyYearsModel();
            var myparam = new DynamicParameters();
            myparam.Add("_UserId", currentUserService.UserNumericId, DbType.Int64);
            myparam.Add("YearStudy", YearStudy, DbType.Int64);
            myparam.Add("CourseID", CourseId, DbType.Int64);
            var result = await dapper.MultiResult("usp_VerifyYearStatus", myparam, commandType: CommandType.StoredProcedure);
            if (result[0] != null)
            {
                var data = JsonConvert.SerializeObject(result[0]);
                obj.yearsStatus = JsonConvert.DeserializeObject<List<Drp>>(data);
            }
            if (result[1] != null)
            {
                
                var data = JsonConvert.SerializeObject(result[1].FirstOrDefault());
                var res = JsonConvert.DeserializeObject<IsValidResult>(data);
    ...
    public async Task<int> CheckIsCurrentAvailable()
    {
        try
        {
            ...
            var myparam = new DynamicParameters();
            myparam.Add("_UserID", currentUserService.UserNumericId, DbType.Int32);
            var result = await Task.FromResult(dapper.Get<int>("usp_CheckIsCurrentAvailable", myparam, commandType: System.Data.CommandType.StoredProcedure));
    ...
    public async Task<string> GetVerifyHostel(int district)
    {
        try
        {
            var myparam = new DynamicParameters();
            myparam.Add("p_UserId", currentUserService.UserNumericId, DbType.Int32);
            myparam.Add("p_DistrictId", district, DbType.Int32);
            var result = await Task.FromResult(dapper.Get<string>("usp_GetHostelPresentBasedOnDistrictTaluka", myparam, commandType: System.Data.CommandType.StoredProcedure));
            return result;
        }
        catch (Exception ex)
        {
            await errorLogger.Log("usp_GetHostelPresentBasedOnDistrictTaluka", ex);
            throw;
        }
    }

    public async Task<string> UpdateServiceType(int district)
    {
        try
        {
            var myparam = new DynamicParameters();
            myparam.Add("p_UserId", currentUserService.UserNumericId, DbType.Int32);
            myparam.Add("p_districtID", district, DbType.Int32);
            var result = await Task.FromResult(dapper.Get<string>("usp_UpdateServiceTypeInPersonalDetails", myparam, commandType: System.Data.CommandType.StoredProcedure));
            return result;
        }
        catch (Exception ex)
        {
            await errorLogger.Log("usp_UpdateServiceTypeInPersonalDetails", ex);
            throw;
        }
    }
