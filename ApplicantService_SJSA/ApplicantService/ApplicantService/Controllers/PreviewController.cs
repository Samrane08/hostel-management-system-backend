    ...
    public class PreviewController : APIBaseController
    {
        ...
        public async Task<IActionResult> Get()
        {
            try
            {
                ...
                if (personalDetails.IsNewApplicant == true)
                {
                    if (ServiceType == 1 && CheckServiceEnable.HostelNewEnable == false)
                    {
                        return Ok(new { Status = false, Message = "New Hostel apply date has been expired." });
                    }
                }
                else if (personalDetails.IsNewApplicant == false)
                {
                    if (ServiceType == 1 && CheckServiceEnable.HostelExistingEnable == false)
                    {
                        return Ok(new { Status = false, Message = "Existing Hostel apply date has been expired." });
                    }
                }

                bool IsApplicationExist = await profileService.CheckApplicationAlreadyExistForService(ServiceType);
                ...
                var validate = await profileService.ProfileStatus();
                ...
                {
                    return Ok(new { Status = false, Message = "Current qualification is not updated." });
                }
