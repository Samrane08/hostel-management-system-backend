...
namespace ApplicantService.Controllers
{
    public class CurrentCourseController : APIBaseController
    {
        ...
        public async Task<IActionResult> SaveCurrentQualification(CurrentCourseDetails model)
        {
            try
            {
                ...
                if (saveResult.Split(":")[0] != "0")
                {
                    ...
                    return Ok(new { status = false, message = saveResult.Split(":")[1], Data = model });
        ...
        public async Task<object> GetQualificationByuserId()
        {
            try
            {
                List<CourseDetailsTblViewModel> result = null;
                 result = await currentcourseService.GetCourseDetailsByUserId();
               if(result == null || result.Count<1)
                {
                    string UIDINo = ""; string FirstDigit = "";
                    var registerDetails = await currentcourseService.GetAadharAndCheckShouldFetchRecordFromPost("current");
                    if (!string.IsNullOrWhiteSpace(registerDetails))
                    {
                        ...
                        var postResult = await httpClientService.RequestSendWithHeader<PostApiModel>(HttpMethod.Post, $"{urloptions.PostService}/hms/GetHMS_CurrentCourseData", PostReqModel);
                        if (postResult != null && postResult.CurrentCourseDetails != null && postResult.CurrentCourseDetails.Count > 0)
                        {
                            foreach (var o in postResult.CurrentCourseDetails)
                            {
                                await currentcourseService.SaveCurrentCourseDetailsAsync(o);
                            }
        ...
        [HttpGet("CheckIsDatainOrder")]
        public async Task<IActionResult> CheckIsDatainYearOrder(int CourseId, int YearStudy)
        {
            try
            {
